        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (IsCategoryId == true && IsSubCategoryId == true && IsBrandId == true && Isname == true)
            {
                if (BrandId == 0)
                {
                    if (subCategoryId == 0)
                    {
                        productPriceList.AddRange((from p in entity.ProductPriceChanges where p.Product.ProductCategoryId == CategoryId && p.Product.ProductSubCategoryId == null && p.Product.BrandId == null && p.Product.Name == name select p).ToList());
                    }
                    else
                    {
                        productPriceList.AddRange((from p in entity.ProductPriceChanges where p.Product.ProductCategoryId == CategoryId && p.Product.ProductSubCategoryId == subCategoryId && p.Product.BrandId == null && p.Product.Name == name select p).ToList());
                    }
                }
                else
                {
                    if (subCategoryId == 0)
                    {
                        productPriceList.AddRange((from p in entity.ProductPriceChanges where p.Product.ProductCategoryId == CategoryId && p.Product.ProductSubCategoryId == null && p.Product.BrandId == BrandId && p.Product.Name == name select p).ToList());
                    }
                    else
                    {
                        productPriceList.AddRange((from p in entity.ProductPriceChanges where p.Product.ProductCategoryId == CategoryId && p.Product.ProductSubCategoryId == subCategoryId && p.Product.BrandId == BrandId && p.Product.Name == name select p).ToList());
                    }
                }
            ...
            else if (IsCategoryId == true && IsSubCategoryId == true && IsBrandId == false && Isname == true)
            {
                if (subCategoryId == 0)
                {
                    productPriceList.AddRange((from p in entity.ProductPriceChanges where p.Product.ProductCategoryId == CategoryId && p.Product.ProductSubCategoryId == null && p.Product.Name == name select p).ToList());
                }
                else
                {
                    productPriceList.AddRange((from p in entity.ProductPriceChanges where p.Product.ProductCategoryId == CategoryId && p.Product.ProductSubCategoryId == subCategoryId && p.Product.Name == name select p).ToList());
                }
            ...
            else if (IsCategoryId == false && IsSubCategoryId == false && IsBrandId == true && Isname == true)
            {
                if (BrandId == 0)
                {
                    productPriceList.AddRange((from p in entity.ProductPriceChanges where p.Product.BrandId == null && p.Product.Name == name select p).ToList());
                }
                else
                {
                    productPriceList.AddRange((from p in entity.ProductPriceChanges where p.Product.BrandId == BrandId && p.Product.Name == name select p).ToList());
                }
            ...
            {
                productPriceList.AddRange((from p in entity.ProductPriceChanges where p.Product.Name == name select p).ToList());
                foundDataBind();
            }
            ...
            else if (IsCategoryId == true && IsSubCategoryId == false && IsBrandId == true && Isname == true)
            {
                if (BrandId == 0)
                {
                    productPriceList.AddRange((from p in entity.ProductPriceChanges where p.Product.ProductCategoryId == CategoryId && p.Product.BrandId == null && p.Product.Name == name select p).ToList());
                }
                else
                {
                    productPriceList.AddRange((from p in entity.ProductPriceChanges where p.Product.ProductCategoryId == CategoryId && p.Product.BrandId == BrandId && p.Product.Name == name select p).ToList());
                }
            ...
            {
                productPriceList.AddRange((from p in entity.ProductPriceChanges where p.Product.ProductCategoryId == CategoryId && p.Product.Name == name select p).ToList());
                foundDataBind();
            }
