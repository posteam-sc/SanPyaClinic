...
namespace POS
{
    public partial class TransactionReport_FOC_MPU : Form
    {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            if (rdbSummary.Checked == false)
            {
                ...
                foreach (Transaction transaction in transList)
                {
                    ...
                    newRow.Date = transaction.DateTime.ToString();
            ...
            {
                int totalSale = 0, totalRefund = 0, totalDebt = 0, totalCreditRefund = 0, totalSummary = 0; int totalGiftCard = 0, totalCredit = 0, totalCreditRecieve = 0, totalCashInHand = 0, totalExpense = 0, totalIncomeAmount = 0, totalMPU = 0, totalFOC = 0, totalReceived = 0; long totalDiscount = 0, totalRefundDiscount = 0, totalCreditRefundDiscount = 0,Sc=0,Cf=0,Injec=0;
                #region Transaction
                dsReportTemp dsReport = new dsReportTemp();
                dsReportTemp.TransactionListDataTable dtTransactionReport = (dsReportTemp.TransactionListDataTable)dsReport.Tables["TransactionList"];
                ...
                {
                    long itemdiscount = (long)t.TransactionDetails.Where(x => x.IsDeleted==false).Sum(x => (x.UnitPrice * (x.DiscountRate / 100)) * x.Qty);
        ...
        private void ShowReportViewer1()
        {
            ...
            List<Transaction> doctortransList = new List<Transaction>();
            ...
            foreach (Transaction t in doctortransList)
            {
                Sc += (long)t.ServiceChages;
                Cf += (long)t.Consultantfees;
                Injec += (long)t.TransactionDetails.Where(x => x.IsDeleted==false).Sum(x => (x.InjectionRate));
            }
            #endregion
            #region Refund
            dsReportTemp dsReportRefund = new dsReportTemp();
            ...
            //totalSummary = (totalSale + totalDebt + totalCreditRefund + totalGiftCard) - totalRefund;
            totalSummary = ((totalSale + totalCredit + totalGiftCard+totalMPU) - (totalRefund + totalCreditRefund+totalFOC));
            totalCashInHand = (totalSale + totalDebt + totalCreditRecieve) - Convert.ToInt32((totalRefund  + Cf + Injec));
            totalExpense = (totalRefund + totalCreditRefund+totalFOC);
            totalIncomeAmount = (totalSale + totalCredit + totalGiftCard+totalMPU);
            totalReceived = (totalSale + totalDebt + totalCreditRecieve);
            string reportPath = Application.StartupPath + "\\Reports\\TransactionsDetailReport.rdlc";
