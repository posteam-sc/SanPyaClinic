    ...
    public partial class ProductSubCategory : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (MainCategoryId > 0)
            {
                ...
                else if (txtPrefix.Text.Trim() == string.Empty)
                {
                    tp.SetToolTip(txtPrefix, "Error");
                    tp.Show("Please fill up Sub Category Code!", txtPrefix);
                    hasError = true;
                }
                else if (txtPrefix.Text.Length <= 1)
                {
                    tp.SetToolTip(txtPrefix, "Error");
                    tp.Show("Please fill up Sub Category Code at least 2 character!", txtPrefix);
                    hasError = true;
                }
                else if (txtPrefix.Text.Length > 4)
                {
                    tp.SetToolTip(txtPrefix, "Error");
                    tp.Show("Please fill up Sub Category Code at Maximun 4 character!", txtPrefix);
                    hasError = true;
                }
                if (hasError == false)
                {
                    ...
                    if (!isEdit)
                    {
                        ...
                        {
                            APP_Data.ProductSubCategory pSubCat = (from psCat in posEntity.ProductSubCategories where psCat.Name == txtName.Text && psCat.ProductCategoryId == MainCategoryId && psCat.Prefix == txtPrefix.Text select psCat).FirstOrDefault();
                            ...
                            if (pSubCat == null)
                            {
                                if (pSubCat2 == null)
                                {
                                    ...
                                    CategoryId=Convert.ToInt32( pTypeObj.ProductCategoryId);
                                    SubCategoryId = pTypeObj.Id;
                                    dgvProductList.DataSource = (from pType in posEntity.ProductSubCategories orderby pType.Id descending select pType).ToList();
                        ...
                        if (controller.SubCategory.Add || MemberShip.isAdmin)
                        {
                            ...
                            if (psc == null)
                            {
                                ...
                                if (p == null)
                                {
                                    ...
                                    CategoryId = Convert.ToInt32(EditCat.ProductCategoryId);
                                    SubCategoryId = EditCat.Id;
                ...
                if (e.ColumnIndex ==5 )
                {
                    ...
                    if (controller.SubCategory.EditOrDelete || MemberShip.isAdmin)
                    {
                        ...
                        if (result.Equals(DialogResult.OK))
                        {
                            ...
                            {
                                //To show message box 
                                MessageBox.Show("This product type is currently in use!", "Enable to delete", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void txtPrefix_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                tp.SetToolTip(txtPrefix, "Error");
                tp.Show("Please fill up character!", txtPrefix);
        ...
        private void ProductSubCategory_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (System.Windows.Forms.Application.OpenForms["NewProduct"] != null)
            {
                ...
                newForm.SetCurrentSubCategory(SubCategoryId);
            }
            #endregion
        }
