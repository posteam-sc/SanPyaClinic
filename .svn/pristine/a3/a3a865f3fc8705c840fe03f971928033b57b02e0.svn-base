    ...
    public partial class PaidByGiftCard : Form
    {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (!hasError)
            {
                ...
                if (Type == "Doctor")
                {
                    Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Sale, true, true, 3, ExtraTax + Tax, ExtraDiscount + Discount, TotalCost, TotalCost, currentCard.Id, CustomerId, DId, SC,null,Cfees);
                    resultId = Id.FirstOrDefault().ToString();
                }
                else if (Type == "Physio")
                {
                    Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Sale, true, true, 3, ExtraTax + Tax, ExtraDiscount + Discount, TotalCost, TotalCost, currentCard.Id, CustomerId, DId, SC, PId, Cfees);
                    resultId = Id.FirstOrDefault().ToString();
                ...
                {
                    Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Sale, true, true, 3, ExtraTax + Tax, ExtraDiscount + Discount, TotalCost, TotalCost, currentCard.Id, CustomerId, null, SC, null, Cfees);
                    resultId = Id.FirstOrDefault().ToString();
                }
