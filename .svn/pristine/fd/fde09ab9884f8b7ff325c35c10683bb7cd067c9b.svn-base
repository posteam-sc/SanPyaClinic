...
namespace POS
{
    public partial class PaidByFOC : Form
    {
        ...
        private void PaidByFOC_Load(object sender, EventArgs e)
        {
            ...
            if (Type == "Doctor")
            {
                Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Sale, true, true, 4, ExtraTax + Tax, ExtraDiscount + Discount, totalCost, 0, null, CustomerId, DId, SC,null,Cfees);
                resultId = Id.FirstOrDefault().ToString();
            }
            else if (Type == "Physio")
            {
                Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Sale, true, true, 4, ExtraTax + Tax, ExtraDiscount + Discount, totalCost, 0, null, CustomerId, DId, SC, PId, Cfees);
                resultId = Id.FirstOrDefault().ToString();
            ...
            {
                Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Sale, true, true, 4, ExtraTax + Tax, ExtraDiscount + Discount, totalCost, 0, null, CustomerId, null, SC, null, Cfees);
                resultId = Id.FirstOrDefault().ToString();
            }
